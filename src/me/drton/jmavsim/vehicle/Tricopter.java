...
/**
 * Generic tricopter model.
 */
public class Tricopter extends AbstractMulticopter {
    private static final int rotorsNum = 3;
    private Vector3d[] rotorPositions = new Vector3d[rotorsNum];

    /**
     * Generic tricopter constructor.
     *
     ...
     */
    public Tricopter(World world, String modelName, String orientation, double armLength, double rotorThrust,
                      double rotorTorque, double rotorTimeConst, Vector3d rotorsOffset) {
        super(world, modelName);
        rotorPositions[0] = new Vector3d(0.19304, 0.0, 0.0);
        rotorPositions[1] = new Vector3d(-armLength * Math.cos(Math.PI / 3), -armLength * Math.sin(Math.PI / 3), 0.0);
        ...
        for (int i = 0; i < rotors.length; i++) {
            ...
            rotor.setFullTorque((i == 0 || i == 2) ? rotorTorque : -rotorTorque);
